...
public class JPEGLRGBReader implements PTMReader {
	...
	private boolean reset = true; /* assume a complete input stream */
        ...
        Logger log = LogManager.getLogger(isf.ptm.fileio.JPEGLRGBReader.class);
    ...
    private PTM _initRead() throws IOException, NumberFormatException {
       ...
       // debug("height: " + height);
        ...
        /* scale */
        String[] sa = Utils.split(PTMIO.getLine(in)," ");
        float[] scale = new float[sa.length];
        for (int i = 0; i < sa.length; i++)
                scale[i] = Float.parseFloat(sa[i]);

        /* bias */
        sa = Utils.split(PTMIO.getLine(in)," ");
               ...
               // debug("index: " + index);
                if (referencePlane[index] < 0) {
                       ...
                       // debug(" > ref invert+combine w/" + referencePlane[index]);
                        ...
                        log.error("Error, unhandled format");
                        return null;
                }
        ...
        for (int h = height - 1; h >= 0; h--) {
                for (int w = 0; w < ptm.getWidth(); w++) {
                        ...
                        coefficients[5][off] = (int) PTMIO.cFinal(coef[5][off],
                                        bias[5], scale[5]);*/
                        coefficients[0][off] = (int) ((coef[0][off]-bias[0])* scale[0]);
                        coefficients[1][off] = (int) ((coef[1][off]-bias[1])* scale[1]);
                        coefficients[2][off] = (int) ((coef[2][off]-bias[2])* scale[2]);
                        coefficients[3][off] = (int) ((coef[3][off]-bias[3])* scale[3]);
                        coefficients[4][off] = (int) ((coef[4][off]-bias[4])* scale[4]);
                        coefficients[5][off] = (int) ((coef[5][off]-bias[5])* scale[5]);
                }
        }
