 ...
 * EnvironmentMap.java
 *
 * Created on September 11, 2004, 11:09 PM
 */
...
public class EnvironmentMap {
	...
	private static VRGB eye = new VRGB(0, 0, 1);
	...
	public void setImage(BufferedImage source) {
		if (source == null) {
			original = new BufferedImage(original.getWidth(), original
		...
		this.MAX_DOWNSAMPLE_SZ = Math.min(original.getWidth() / 16, original
				.getHeight() / 16);
		// refresh();
		this.refresh();
		this.setAngle(0);
	 ...
	 */
	public int getPixel(VRGB normal) {
		VRGB R = VRGB.reflect(normal, eye);
		R.normalize();
		// float m = (float) Math.sqrt(2*(R.b+1));
		float m = (float) (Math.sqrt(sqr(R.red) + sqr(R.green) + sqr(R.blue + 1)));
		float u = R.red / m; /* mirror */
		float v = -R.green / m;

		int u1 = Math.max(Math.min(Math.round((u + 1) * width / 2), width - 1),
				0);
		int v1 = Math.max(Math
				.min(Math.round((v + 1) * height / 2), height - 1), 0);
	 ...
	 */
	public Point getPosition(VRGB normal) {
		if (normal == null)
			return null;
	 ...
	 *         normal
	 */
	public int getMapIndex(VRGB normal) {
		VRGB R = VRGB.reflect(normal, eye);
		R.normalize();
		// float m = (float) Math.sqrt(2*(R.b+1)); /* less standard */
		float m = (float) (Math.sqrt(sqr(R.red) + sqr(R.green) + sqr(R.blue + 1)));
		float u = R.red / m; /* mirror */
		float v = -R.green / m;
		int u1 = Math.max(Math.min(Math.round((u + 1) * width / 2), width - 1),
	...
	public void refresh() {
		...
		__pixels = null;
		__pixels = ((DataBufferInt) ((BufferedImage) image).getRaster()
	 ...
	 */
	public void updateMonitor(VRGB[] normals) {
