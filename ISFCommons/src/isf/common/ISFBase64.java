...
public class ISFBase64
{
    static Logger log = LogManager.getLogger(isf.common.ISFBase64.class);
    public static class OutputStream extends FilterOutputStream
    {
            ...
            throws IOException
        {
            ...
            if(encode)
            {
                ...
                {
                    super.out.write(ISFBase64.encode3to4(b4, buffer, bufferLength));
                    lineLength += 4;
            ...
            if(ISFBase64.DECODABET[theByte & 0x7f] > -5)
            {
                buffer[position++] = (byte)theByte;
                if(position >= bufferLength)
                {
                    int len = ISFBase64.decode4to3(buffer, 0, b4, 0);
            ...
            throws IOException
        {
            if(position > 0)
            {
                if(encode)
                {
                    super.out.write(ISFBase64.encode3to4(b4, buffer, position));
                    position = 0;
                } else
                {
            ...
            throws IOException
        {
            if(position < 0)
            {
                if(encode)
                {
                    ...
                    if(numBinaryBytes > 0)
                    {
                        ISFBase64.encode3to4(b3, 0, numBinaryBytes, buffer, 0);
                        position = 0;
                        numSigBytes = 4;
                    } else
                    {
                ...
                } else
                {
                    ...
                    for(i = 0; i < 4; i++)
                    {
                        ...
                        {
                            b = super.in.read();
                        } while(b >= 0 && ISFBase64.DECODABET[b & 0x7f] <= -5);
                        if(b < 0)
                        {
                            break;
                        }
                    ...
                    if(i == 4)
                    {
                        numSigBytes = ISFBase64.decode4to3(b4, 0, buffer, 0);
                        position = 0;
                    } else
                    if(i == 0)
                    {
    ...
    public static byte[] decode(String s)
    {
        ...
        if(bytes.length >= 2)
        {
            ...
            if(bytes != null && bytes.length >= 4 && 35615 == head)
            {
                ...
                catch(IOException ioexception) { log.error(ioexception); }
                finally
                {
                    try
                    {
                    ...
                    catch(Exception exception1) { log.error(exception1);}
                    try
                    {
                        gzis.close();
                    }
                    catch(Exception exception2) {log.error(exception2); }
                    try
                    {
                        bais.close();
                    }
    ...
    public static String getVal(String s)
    {
        if(s != null)
        {
            for(int p = s.indexOf("&"); p >= 0; p = s.indexOf("&", p + 1))
            {
    ...
    public static String getQueryVal(String s)
    {
        if(s != null)
        {
            for(int p = s.indexOf("&"); p >= 0; p = s.indexOf("&", p + 33))
            {
                s = s.substring(0, p) + "&apos;||&apos;&amp;&apos;||&apos;" + s.substring(p + 1);
            }

            for(int p = s.indexOf("'"); p >= 0; p = s.indexOf("'", p + 12))
            {
                s = s.substring(0, p) + "&apos;&apos;" + s.substring(p + 1);
            }
